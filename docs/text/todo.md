...
- [ ] Codebase hygiene (high priority):
  ...
  - [ ] Break plotting into focused modules (`tree_seg/visualization/plotting.py`) for composites, metrics charts, and panel layouts. (Edge/side-by-side now live in `visualization/overlay.py`; eval panels moved to `visualization/eval_panels.py`; exported via `visualization/__init__.py`.)
