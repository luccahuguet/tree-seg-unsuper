...
from tree_seg.tree_focus.vegetation_indices import excess_green_index, create_vegetation_mask


def analyze_vegetation_features(
    image_path: str,
    k_value: int = 15,
    output_dir: str = "data/output/feature_analysis"
):
    """
    ...
    for label in unique_labels:
        ...
        print(f"  Cluster {label:2d}: {category:8s} | "
    ...
    if len(veg_clusters) > 0:
        ...
        print(f"  Total vegetation pixels (via clusters): {total_veg_pixels:,} "
    ...
    if len(veg_clusters) > 0:
        ...
        axes[1, 1].set_title(f"Vegetation Filter (K-means + ExG)\n"
                            f"{len(veg_clusters)} clusters, "
                            f"{100*total_veg_pixels/labels.size:.1f}% of image")
    else:
        axes[1, 1].text(0.5, 0.5, "No vegetation clusters found",
    ...
    if len(cluster_stats) > 1:
        ...
        print(f"Correlation between ExG and vegetation coverage: {corr:.3f} (p={p_value:.4f})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="data/output/feature_analysis",
