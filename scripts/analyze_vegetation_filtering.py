...
def evaluate_v3_1_sample(
    ...
    # Initialize configs
    config_v1_5 = Config(
        pipeline="v1_5", auto_k=False, n_clusters=k_value, verbose=False
    )
    ...
    for i, idx in enumerate(sample_indices):
        ...
        result = {
            ...
            "removed_clusters": results_v1_5.n_clusters_used
            - results_v3_1.n_clusters_used,
            ...
            "removed_pct": float(100 * removed_pixels / v1_5_pixels)
            if v1_5_pixels > 0
            else 0.0,
        }
        ...
        print(
            f"  V1.5: {result['v1_5_clusters']} clusters, {result['v1_5_coverage_pct']:.1f}% coverage"
        )
        print(
            f"  V3.1: {result['v3_1_clusters']} clusters, {result['v3_1_coverage_pct']:.1f}% coverage"
        )
        print(
            f"  Removed: {result['removed_pct']:.1f}% ({result['removed_pixels']:,} px)"
        )
    ...
    aggregate = {
        ...
        "avg_removed_clusters": float(
            np.mean([r["removed_clusters"] for r in results])
        ),
    ...
    with open(output_json, "w") as f:
        json.dump({"aggregate": aggregate, "per_sample": results}, f, indent=2)
    ...
    print(
        f"  Avg removed: {aggregate['avg_removed_pct']:.1f}% ± {aggregate['std_removed_pct']:.1f}%"
    )
...
def generate_sample_visualization(
    image_np: np.ndarray, results_v1_5, results_v3_1, image_id: int, output_path: Path
):
    ...
    axes[0, 1].contour(
        boundaries, levels=[0.5], colors="white", linewidths=1, alpha=0.8
    )
    ...
    veg_boundaries = skimage_seg.find_boundaries(
        results_v3_1.labels_resized, mode="thick"
    )
    axes[1, 0].contour(
        veg_boundaries, levels=[0.5], colors="lime", linewidths=1.5, alpha=0.9
    )
    ...
    if removed_mask.any():
        display_image[removed_mask] = (
            display_image[removed_mask] * 0.5 + np.array([128, 0, 0]) * 0.5
        )
    ...
    if removed_mask.any():
        ...
        removed_boundaries = skimage_seg.find_boundaries(
            removed_labels.astype(int), mode="thick"
        )
        axes[1, 1].contour(
            removed_boundaries,
            levels=[0.5],
    ...
    removed_pct = (
        100 * removed_mask.sum() / v1_5_mask.sum() if v1_5_mask.sum() > 0 else 0
    )
