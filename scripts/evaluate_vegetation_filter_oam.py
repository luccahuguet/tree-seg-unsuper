...
def evaluate_vegetation_filter(
    ...
    for sample in tqdm(samples, desc="Evaluating samples"):
        ...
        try:
            ...
            unique_mask_values = sorted(
                [v for v in np.unique(gt_instance_mask) if v > 0]
            )

            # Map mask values to annotations
            for i, mask_value in enumerate(unique_mask_values):
                if (
                    i < len(coco_annotations)
                    and coco_annotations[i]["category_id"] == 1
                ):
            ...
            results.append(
                {
                    ...
                    "f1": float(f1),
                    "n_gt_trees": int(
                        sum(1 for ann in coco_annotations if ann["category_id"] == 1)
                    ),
    ...
    print()
    print(
        "⚠️  NOTE: OAM-TCD has incomplete annotations (many unannotated trees/vegetation)"
    )
    ...
    print(
        f"  Overall Recall: {overall_recall:.1%} ({total_tp_pixels:,} / {total_gt_pixels:,} GT tree pixels)"
    )
    print(
        f"  Mean Recall:    {avg_recall:.1%} ± {np.std([r['recall'] for r in results]):.1%}"
    )
    ...
    print(
        f"  High (≥80%):   {high_recall:3d} images ({100 * high_recall / len(results):.1f}%)"
    )
    print(
        f"  Medium (30-80%): {medium_recall:3d} images ({100 * medium_recall / len(results):.1f}%)"
    )
    print(
        f"  Low (<30%):    {low_recall:3d} images ({100 * low_recall / len(results):.1f}%)"
    )
    print(
        f"  Zero (0%):     {zero_recall:3d} images ({100 * zero_recall / len(results):.1f}%) ⚠️"
    )
    print()

    if zero_recall > 0:
        print(
            f"⚠️  {zero_recall} images with 0% recall - trees filtered out as non-vegetation"
        )
        print(
            "    Possible causes: very dark trees, heavy shadows, dead/brown vegetation"
        )
        print()
    ...
    print("Best Recall:")
    print(
        f"  Image {best_recall['image_id']}: {best_recall['recall']:.3f} recall ({best_recall['n_gt_trees']} trees)"
    )
    print()

    print("Worst Recall:")
    print(
        f"  Image {worst_recall['image_id']}: {worst_recall['recall']:.3f} recall ({worst_recall['n_gt_trees']} trees)"
    )
    print()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate V3 vegetation filter on OAM-TCD"
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        "--threshold", type=float, default=0.10, help="ExG threshold for vegetation"
    )
    parser.add_argument(
        "--no-auto-k", action="store_true", help="Disable auto K selection"
    )
    parser.add_argument(
        "--elbow", type=float, default=5.0, help="Elbow threshold for auto K"
    )
    parser.add_argument(
        "--output",
        type=str,
