...
def compare_results():
    ...
    # Load both datasets
    pre_auth = load_jsonl("data/outputs/performance_log_pre_auth.jsonl")
    with_auth = load_jsonl("data/outputs/performance_log.jsonl")
    ...
    print("-" * 80)
    print(
        f"{'Model':<12} {'Pre-Auth (s)':<12} {'With-Auth (s)':<14} {'Difference':<12} {'WCSS Diff':<12}"
    )
    print("-" * 80)
    ...
    for model_name in [
        ...
        if model_name in pre_auth_dict and model_name in with_auth_dict:
            ...
            print(
                f"{model_short:<12} {pre['total_time_s']:<12.3f} {auth['total_time_s']:<14.3f} {time_change:<12} {wcss_change:<12}"
            )
    ...
    if total_time_diff > 0:
        print(
            f"• Processing time: +{total_time_diff:.3f}s total ({total_time_diff / 4:.3f}s avg)"
        )
