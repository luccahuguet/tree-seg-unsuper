...
def load_benchmark_data():
    ...
    log_file = Path("data/outputs/performance_log.jsonl")
...
def analyze_performance():
    ...
    df = df.with_columns(
        [
            pl.col("model_name")
            .map_elements(lambda x: model_order.index(x) if x in model_order else 999)
            .alias("sort_order")
        ]
    ).sort("sort_order")
    ...
    print("-" * 80)
    print(
        f"{'Model':<18} {'Params':<8} {'Dims':<6} {'Total':<8} {'K-Sel':<8} {'Cluster':<8} {'K':<3}"
    )
    print("-" * 80)
    ...
    for row in df.iter_rows(named=True):
        ...
        print(
            f"{model_short:<18} {row['model_params']:<8} {row['feature_dims']:<6} "
    ...
    df = df.with_columns(
        [
            ...
            pl.col("model_params")
            .map_elements(
                lambda x: float(x.replace("B", "000").replace("M", ""))
                * (1000 if "B" in x else 1)
            )
    ...
    for row in df.iter_rows(named=True):
        ...
        print(
            f"{model_short:<12} {row['time_ratio']:.1f}x time  {row['param_ratio']:.1f}x params  "
    ...
    k_selection_pct = (
        df.select(pl.col("k_selection_s").mean()).item()
        / df.select(pl.col("total_time_s").mean()).item()
    ) * 100

    feature_dims_range = df.select(
        [pl.col("feature_dims").min(), pl.col("feature_dims").max()]
    )

    print(f"• K-selection dominates: {k_selection_pct:.1f}% of total time")
    print("• All models converged to K=8 for this forest image")
    print(
        f"• Feature dimensions scale: {feature_dims_range.item(0, 0)}D → {feature_dims_range.item(0, 1)}D"
    )
    ...
    print(
        f"• Performance scaling: {param_scaling:.0f}x params → {time_scaling:.1f}x time"
    )
