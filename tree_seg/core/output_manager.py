...
class OutputManager:
    ...
    def generate_filename_prefix(
        self, image_path: str, n_clusters_used: int, requested_k: int | None = None
    ) -> str:
        """
        ...
        components = [file_hash, model_nick, f"str{self.config.stride}"]

        # Add clustering method info
        if self.config.auto_k:
            # Format elbow threshold without float artifacts (e.g., 3.5)
            et_clean = (
                f"{self.config.elbow_threshold:.2f}".rstrip("0").rstrip(".")
            ).replace(".", "-")
    ...
    def generate_output_paths(
        ...
        paths = OutputPaths(
            ...
            elbow_analysis=str(self.png_dir / f"{prefix}_elbow_analysis.png")
            if include_elbow
            else None,
        )
    ...
    def find_latest_outputs(self) -> Optional[OutputPaths]:
        ...
        if not any(
            [
                latest_files["segmentation_legend"],
        ...
        try:
            ...
            with Image.open(input_path) as img:
                ...
                if img.width > self.config.web_max_width:
                    ...
                    img = img.resize(
                        (self.config.web_max_width, new_height),
                        Image.Resampling.LANCZOS,
                    )

                # Save as optimized JPEG
                img.save(
                    output_file, "JPEG", quality=self.config.web_quality, optimize=True
                )
                ...
                print(
                    f"   {original_size:.1f}MB → {optimized_size:.1f}MB ({optimized_size / original_size * 100:.0f}%)"
                )
    ...
    def optimize_all_outputs(self, output_paths: OutputPaths) -> OutputPaths:
        ...
        optimized = OutputPaths(
            segmentation_legend=self.optimize_image_for_web(
                output_paths.segmentation_legend
            )
            or output_paths.segmentation_legend,
            edge_overlay=self.optimize_image_for_web(output_paths.edge_overlay)
            or output_paths.edge_overlay,
            side_by_side=self.optimize_image_for_web(output_paths.side_by_side)
            or output_paths.side_by_side,
            elbow_analysis=self.optimize_image_for_web(output_paths.elbow_analysis)
            if output_paths.elbow_analysis
            else None,
        )
