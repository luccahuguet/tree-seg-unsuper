...
def find_optimal_k_elbow(
    features_flat: np.ndarray, k_range: Tuple[int, int] = (2, 15)
) -> Tuple[int, Dict]:
    """
    ...
    elbow_idx = (
        np.argmax(np.abs(second_diff)) + 1
    )  # +1 because we lost 2 points in diff
...
def find_optimal_k(
    features_flat: np.ndarray, k_range: Tuple[int, int] = (2, 15), method: str = "elbow"
) -> Tuple[int, Dict]:
    """
    ...
    if method == "elbow":
        ...
        silhouette_k, silhouette_scores = find_optimal_k_silhouette(
            features_flat, k_range
        )
        ...
        raise ValueError(
            f"Unknown method: {method}. Use 'elbow', 'silhouette', or 'both'"
        )
...
def plot_k_selection_analysis(scores: Dict, output_dir: str, output_prefix: str):
    ...
    if method == "elbow":
        ...
        if silhouette_scores:
            ...
            optimal_idx = (
                k_values.index(silhouette_k) if silhouette_k in k_values else 0
            )
        ...
        plot_path = os.path.join(
            output_dir, f"{output_prefix}_k_selection_analysis.png"
        )
        plt.savefig(plot_path, dpi=300, bbox_inches="tight")
        plt.close()
...
def validate_k_selection(optimal_k: int, k_range: Tuple[int, int], method: str) -> int:
    ...
    if optimal_k < min_k:
        print(
            f"Warning: Selected K={optimal_k} is below minimum {min_k}. Using K={min_k}"
        )
        return min_k
    elif optimal_k > max_k:
        print(
            f"Warning: Selected K={optimal_k} is above maximum {max_k}. Using K={max_k}"
        )
        return max_k
    else:
        return optimal_k
